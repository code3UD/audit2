<?php
/**
 * Page de démonstration du Wizard Moderne
 * Permet de tester le wizard sans Dolibarr
 */

// Simuler l'environnement Dolibarr minimal
if (!function_exists('GETPOST')) {
    function GETPOST($paramname, $check = 'alphanohtml') {
        return $_GET[$paramname] ?? $_POST[$paramname] ?? '';
    }
}
...
if ($action == 'create_audit') {
    ...
    // Simuler la sauvegarde
    echo "<script>alert('Audit créé avec succès! Score: " . round($total_score) . "/100'); window.location.href='?step=6';</script>";
...
<head>
    ...
    <title>Démo - Wizard Audit Digital Moderne</title>
...
<div class="demo-header">
    <h1><i class="fas fa-flask"></i> DÉMONSTRATION - Wizard Audit Digital Moderne</h1>
    <p>Version complète avec toutes les étapes implémentées</p>
</div>
...
<div class="modern-wizard-container">
    ...
    <div class="wizard-content">
        <form id="wizardForm" method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
            ...
            <?php
            // Inclure le contenu des étapes depuis le wizard moderne
            $wizard_content = file_get_contents(__DIR__ . '/wizard/modern.php');
            
            // Extraire le contenu des étapes
            if ($step == 1) {
                preg_match('/<!-- Étape 1: Informations Générales -->(.*?)(?=<\?php elseif)/s', $wizard_content, $matches);
                if (isset($matches[1])) {
                    echo $matches[1];
                }
            } elseif ($step == 2) {
                preg_match('/<!-- Étape 2: Maturité Digitale -->(.*?)(?=<\?php elseif)/s', $wizard_content, $matches);
                if (isset($matches[1])) {
                    echo $matches[1];
                }
            } elseif ($step == 3) {
                preg_match('/<!-- Étape 3: Cybersécurité -->(.*?)(?=<\?php elseif)/s', $wizard_content, $matches);
                if (isset($matches[1])) {
                    echo $matches[1];
                }
            } elseif ($step == 4) {
                preg_match('/<!-- Étape 4: Cloud & Infrastructure -->(.*?)(?=<\?php elseif)/s', $wizard_content, $matches);
                if (isset($matches[1])) {
                    echo $matches[1];
                }
            } elseif ($step == 5) {
                preg_match('/<!-- Étape 5: Automatisation -->(.*?)(?=<\?php elseif)/s', $wizard_content, $matches);
                if (isset($matches[1])) {
                    echo $matches[1];
                }
            } elseif ($step == 6) {
                // Étape 6 avec calcul de score
                $wizard_data = $_SESSION['audit_wizard_data'] ?? array();
                $total_score = 0;
                $scores_detail = array();
                ...
                <div class="step-container">
                    ...
                    <div style="text-align: center; padding: 40px;">
                        ...
                        <div style="margin-top: 30px;">
                            <a href="?step=1" style="background: var(--gradient-primary); color: white; padding: 15px 30px; border-radius: 50px; text-decoration: none; display: inline-flex; align-items: center; gap: 10px;">
                                <i class="fas fa-redo"></i> Recommencer un audit
                            </a>
                        </div>
                    </div>
                </div>
                ...
                echo '<div class="step-container"><div class="step-header"><h2>Étape ' . $step . '</h2><p>Contenu en cours de chargement...</p></div></div>';
            }
            ?>
